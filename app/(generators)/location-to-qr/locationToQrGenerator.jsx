...
function validateLon(lon) {
    if (!lon.trim()) return "Longitude is required";
    const n = Number(lon);
    if (isNaN(n) || n < -180 || n > 180) return "Longitude must be between -180 and 180";
    return "";
}

export default function LocationToQrGenerator() {
    ...
    }, [lat, lon, label, errorCorrectionLevel, width, height]);

    const generateQRCode = async () => {
        let data = `geo:${lat.trim()},${lon.trim()}`;
        if (label.trim()) {
            data += `?q=${encodeURIComponent(label.trim())}`;
        }
        try {
    ...
    const downloadQRCode = () => {
        ...
        try {
            ...
            link.download = `location-qr-code-${Date.now()}.png`;
            link.href = qrCodeUrl;
            link.click();
    ...
    const printQRCode = () => {
        ...
        if (printWindow) {
            printWindow.document.write(`
                <html>
                  <head><title>Print Location QR Code</title></head>
    ...
    const shareQRCode = async () => {
        ...
        try {
            if (navigator.share && navigator.canShare) {
                ...
                const file = new File([blob], "location-qr-code.png", { type: "image/png" });
                if (navigator.canShare({ files: [file] })) {
                    await navigator.share({
                        title: "Location QR Code",
                        text: "Scan to open this location",
                        files: [file],
                    });
            ...
            // Fallback: copy geo URI to clipboard
            let data = `geo:${lat.trim()},${lon.trim()}`;
            if (label.trim()) {
                data += `?q=${encodeURIComponent(label.trim())}`;
            }
            navigator.clipboard.writeText(data);
            toast.success("Location URI copied to clipboard!");
        } catch (error) {
    ...
    const isValid = !!qrCodeUrl && !latError && !lonError;
    ...
    return (
        <div className="bg-gradient-to-br from-green-100 via-blue-50 to-indigo-200 pb-12 min-h-screen w-full">
            <div className="grid lg:grid-cols-2 gap-6">
                <Card>
                    <CardHeader>
                        <CardTitle>Location QR Code Settings</CardTitle>
                        <CardDescription>
                            Enter latitude, longitude, and an optional label to generate a QR code for a map location.
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                        <div>
                            ...
                            />
                            {latError && (
                                <div className="text-sm text-red-500 mt-1">{latError}</div>
                            )}
                        </div>
                        <div>
                            <Label htmlFor="lon" className="pb-3">Longitude</Label>
                            <Input
                                id="lon"
                                value={lon}
                            ...
                            />
                            {lonError && (
                                <div className="text-sm text-red-500 mt-1">{lonError}</div>
                            )}
                        </div>
                        <div>
                            <Label htmlFor="label" className="pb-3">Label (optional)</Label>
                            <Input
                                id="label"
                                value={label}
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Generated Location QR Code</CardTitle>
                        <CardDescription>
                            Your location QR code will appear here as you type
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="space-y-4">
                            <div className="flex flex-col items-center p-4 bg-white rounded-lg border">
                                {qrCodeUrl ? (
                                    <img
                                        src={qrCodeUrl}
                                        alt="Generated Location QR Code"
                                        style={{
                                            width: `${width}px`,
                                            height: `${height}px`,
                                            maxWidth: "100%",
