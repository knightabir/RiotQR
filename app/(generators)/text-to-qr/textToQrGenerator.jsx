...
export default function TextToQrGenerator() {
    ...
    function validateText(val) {
        if (!val.trim()) return "Text is required";
        return "";
    }
    ...
    }, [text, errorCorrectionLevel, width, height]);
    ...
    const downloadQRCode = () => {
        ...
        try {
            ...
            link.download = `text-qr-code-${Date.now()}.png`;
            link.href = qrCodeUrl;
            link.click();
    ...
    const printQRCode = () => {
        ...
        if (printWindow) {
            printWindow.document.write(`
                <html>
                  <head><title>Print Text QR Code</title></head>
    ...
    const shareQRCode = async () => {
        ...
        try {
            if (navigator.share && navigator.canShare) {
                ...
                const file = new File([blob], "text-qr-code.png", { type: "image/png" });
                if (navigator.canShare({ files: [file] })) {
                    await navigator.share({
                        title: "Text QR Code",
                        text: "Scan to get this text",
                        files: [file],
                    });
    ...
    const isValid = !!qrCodeUrl && !textError;
    ...
    return (
        <div className="bg-gradient-to-br from-green-100 via-blue-50 to-indigo-200 pb-12 min-h-screen w-full">
            <div className="grid lg:grid-cols-2 gap-6">
                <Card>
                    <CardHeader>
                        <CardTitle>Text QR Code Settings</CardTitle>
                        <CardDescription>
                            Enter text to generate a QR code for plain text.
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                        <div>
                            <Label htmlFor="text" className="pb-3">Text</Label>
                            <Textarea
                                id="text"
                                value={text}
                            ...
                            />
                            {textError && (
                                <div className="text-sm text-red-500 mt-1">{textError}</div>
                            )}
                        </div>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Generated Text QR Code</CardTitle>
                        <CardDescription>
                            Your text QR code will appear here as you type
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
