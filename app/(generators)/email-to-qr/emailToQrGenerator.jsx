...
export default function EmailToQrGenerator() {
    ...
    }, [email, subject, body, errorCorrectionLevel, width, height]);

    const generateQRCode = async () => {
        const data = `mailto:${email.trim()}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
        try {
    ...
    const downloadQRCode = () => {
        ...
        try {
            ...
            link.download = `email-qr-code-${Date.now()}.png`;
            link.href = qrCodeUrl;
            link.click();
    ...
    const printQRCode = () => {
        ...
        if (printWindow) {
            printWindow.document.write(`
                <html>
                  <head><title>Print Email QR Code</title></head>
    ...
    const shareQRCode = async () => {
        ...
        try {
            if (navigator.share && navigator.canShare) {
                ...
                const file = new File([blob], "email-qr-code.png", { type: "image/png" });
                if (navigator.canShare({ files: [file] })) {
                    await navigator.share({
                        title: "Email QR Code",
                        text: "Scan to send an email",
                        files: [file],
                    });
    ...
    const isValid = !!qrCodeUrl && !emailError;
    ...
    return (
        <div className="bg-gradient-to-br from-green-100 via-blue-50 to-indigo-200 pb-12 min-h-screen w-full">
            <div className="grid lg:grid-cols-2 gap-6">
                <Card>
                    <CardHeader>
                        <CardTitle>Email QR Code Settings</CardTitle>
                        <CardDescription>
                            Enter an email address, subject, and message to generate a QR code for sending an email.
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                        ...
                        <div>
                            <Label htmlFor="subject" className="pb-3">Subject (optional)</Label>
                            <Input
                                id="subject"
                                value={subject}
                        ...
                        <div>
                            <Label htmlFor="body" className="pb-3">Message (optional)</Label>
                            <Textarea
                                id="body"
                                value={body}
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Generated Email QR Code</CardTitle>
                        <CardDescription>
                            Your email QR code will appear here as you type
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
