...
export default function EventToQrGenerator() {
    ...
    }, [title, location, description, start, end, errorCorrectionLevel, width, height]);
    ...
    const generateQRCode = async () => {
        ...
        try {
            const qrCodeDataUrl = await QRCode.toDataURL(vevent, {
    ...
    const printQRCode = () => {
        ...
        if (printWindow) {
            printWindow.document.write(`
                <html>
                  <head><title>Print Event QR Code</title></head>
    ...
    const shareQRCode = async () => {
        ...
        try {
            ...
            // Fallback: copy event summary to clipboard
            navigator.clipboard.writeText(title);
            toast.success("Event title copied to clipboard!");
        } catch (error) {
    ...
    const isValid = !!qrCodeUrl && !eventError;
    ...
    return (
        <div className="bg-gradient-to-br from-green-100 via-blue-50 to-indigo-200 pb-12 min-h-screen w-full">
            <div className="grid lg:grid-cols-2 gap-6">
                <Card>
                    <CardHeader>
                        <CardTitle>Event QR Code Settings</CardTitle>
                        <CardDescription>
                            Enter event details to generate a QR code for a calendar event (iCalendar VEVENT).
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                        <div>
                            <Label htmlFor="title" className="pb-3">Event Title</Label>
                            <Input
                                id="title"
                                value={title}
                        ...
                        <div>
                            <Label htmlFor="location" className="pb-3">Location (optional)</Label>
                            <Input
                                id="location"
                                value={location}
                        ...
                        <div>
                            <Label htmlFor="description" className="pb-3">Description (optional)</Label>
                            <Textarea
                                id="description"
                                value={description}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <Label htmlFor="start" className="pb-3">Start Date/Time</Label>
                                <Input
                                    id="start"
                                    type="datetime-local"
                                    value={start}
                            ...
                            <div>
                                <Label htmlFor="end" className="pb-3">End Date/Time</Label>
                                <Input
                                    id="end"
                                    type="datetime-local"
                                    value={end}
                        ...
                        {eventError && (
                            <div className="text-sm text-red-500 mt-1">{eventError}</div>
                        )}
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Generated Event QR Code</CardTitle>
                        <CardDescription>
                            Your event QR code will appear here as you type
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
